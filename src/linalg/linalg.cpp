...
template<typename RT, typename T, bool rf_h>
nd::matrix<RT> nd::linalg::mask_upper(const nd::matrix<T, rf_h> &mat,
		max_t dshift) {
	...
	if (mcoords.ndim < 2) {
		throw nd::exception("nd::linalg::mask_upper(...),\n\t"
				"nd::matrix<T, ...>::ndim() < 2");
	}
	...
	max_t end = std::max(mcoords.nrows(), mcoords.ncols());

	// -------------------------------------------------------------------------------

	for (max_t dsi = dshift; dsi < end; dsi++) {

		DI3_MOVE_ALONG(it, dsi);
...
template<typename RT, typename T, bool rf_h>
nd::matrix<RT> nd::linalg::mask_lower(const nd::matrix<T, rf_h> &mat,
		max_t dshift) {
	...
	if (mcoords.ndim < 2) {
		throw nd::exception("nd::linalg::mask_lower(...),\n\t"
				"nd::matrix<T, ...>::ndim() < 2");
	}
